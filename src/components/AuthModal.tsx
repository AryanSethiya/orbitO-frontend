...
export const AuthModal: FC<AuthModalProps> = ({ isOpen, onClose, onLoginSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[999] flex items-start sm:items-center justify-center p-2.5 sm:p-4 overflow-y-auto min-h-screen py-6 sm:py-8">
      <div className="w-full max-w-md bg-[#0e1410] border border-primary/50 relative p-4 sm:p-8 shadow-[0_0_50px_rgba(72,255,72,0.3)] text-left font-telemetry-md my-auto max-h-[92vh] flex flex-col">
        {/* HUD Corners */}
        <div className="telemetry-corner corner-tl text-primary font-mono text-[9px] sm:text-[10px] truncate max-w-[200px]">
          {step === 'authenticate' ? 'PILOT CLEARANCE' : 'CLAIM CALLSIGN'}
        </div>
        ...
        {error && (
          <div className="p-3 my-3 bg-error-container/30 border border-error text-error text-xs font-telemetry-sm flex items-center gap-2">
          ...
          {step === 'authenticate' ? (
            ...
            <div>
              <div className="mt-2 sm:mt-3 mb-4">
                <div className="font-label-caps text-[11px] sm:text-xs text-primary/80 uppercase tracking-widest mb-1.5 flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-primary animate-pulse"></span>
                  PILOT IDENTIFICATION CLEARANCE
                </div>
                <h2 className="font-display-hero text-xl sm:text-3xl text-white uppercase tracking-tight leading-none">
                  CLAIM CALLSIGN
                </h2>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      maxLength={24}
                      autoFocus
                      className="flex-1 min-w-0 bg-black/90 border border-primary/60 px-3 py-2.5 text-base sm:text-xs font-mono text-primary font-bold uppercase tracking-wider focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary shadow-[0_0_12px_rgba(72,255,72,0.15)] placeholder:text-white/30"
                    />
                    <button
                      type="submit"
                      disabled={loading || !directCallsign.trim()}
                      className="py-2.5 px-3.5 sm:px-4 bg-primary hover:bg-primary/90 text-black font-mono text-xs font-black uppercase tracking-wider transition-all glitch-hover shrink-0 cursor-pointer disabled:opacity-40 shadow-[0_0_20px_rgba(72,255,72,0.35)]"
                    >
              ...
              {/* Google Login Box */}
              <div className="bg-black/50 p-3 sm:p-4 border border-white/10 flex flex-col items-center justify-center text-center overflow-x-hidden">
                {loading ? (
                  ...
                  <div className="w-full max-w-[280px] flex justify-center overflow-hidden">
                    <GoogleLogin
            ...
            <form onSubmit={handleClaimCallsignSubmit} className="space-y-4 pt-2">
              <div>
                <div className="font-label-caps text-xs text-primary uppercase tracking-widest mb-1 flex items-center gap-1.5">
                  <Check className="w-3.5 h-3.5 text-primary" />
                  <span className="truncate">GOOGLE IDENTITY VERIFIED: {pendingUser?.email}</span>
                </div>
                <h2 className="font-display-hero text-xl sm:text-3xl text-white uppercase tracking-tight mb-2">
              ...
              <div className="my-4">
                ...
                <input
                  ...
                  required
                  autoFocus
                  className="w-full bg-black/80 border border-primary/60 px-4 py-3 text-base sm:text-sm font-mono text-primary font-bold uppercase tracking-wider focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary shadow-[0_0_15px_rgba(72,255,72,0.15)]"
                />
